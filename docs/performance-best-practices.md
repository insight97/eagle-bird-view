...
**圖片管線的優化已經完成，不是目前卡頓的原因。** 白板跑在 60 fps 中位數，97% 的畫格間隔為 16.67 ms。
...
- 所有相機移動期間都不啟動新的原圖載入，停下後再按選取／中央素材優先補上清晰度。

**這些行為的不變條件寫在 `AGENTS.md`，改動媒體管線前先看那裡**——有幾條很容易被合理的重構默默破壞。

尚未解決、留給下一位的：

- 殘留的偶發 hitch（34 次／33 秒）**與圖片工作不相關**（重疊率 35% vs 隨機基準 28%）。發生時主執行緒 80% 閒置，形狀是 140–160 ms 的 `RunTask` 但只有一個 0.5 ms 的 `RunMicrotasks` 子事件——主執行緒被某個 Chrome 未產生事件的同步操作擋住，從 trace 無法確定是什麼。
- **下一步應該是 A/B 而不是改 code**：錄一段「原圖全部載入完成後、只做平移」的 trace，期間不應有新的原圖載入。若 hitch 照樣出現，即確定與圖片無關，該轉向 Eagle／Electron 主機、GPU 驅動或素材磁碟。
- `0c56d31`（預載 lead 只服務縮圖）沒有可量測效果，僅憑原則保留。

### 這份文件怎麼讀

下方「第 N 輪」是依時間累積的調查記錄，**保留失敗的嘗試與被推翻的假設是刻意的**——這個主題有好幾個方向看起來合理但實際無效。若只想知道結論，讀本節與最末的「可推廣的判準」。

原本另有一份 `performance-handoff-2026-08-07.md`，其中仍有價值的部分（試過哪些方向、為什麼無效）已併入下方第一輪；其餘內容（當時的 git 狀態、測試數字、程式位置）都已過時，且指向數個現已刪除的函式，因此移除。需要原文可從 git 歷史取得。

## 結論先行（2026-07-27 的原始研究，假設已被第一輪推翻）
...
第一輪定位之前試過以下八種，全部無效或有反效果。列出來是為了避免重走：

| 嘗試 | 結果 |
| --- | --- |
| 移動期間把原圖降回縮圖 | 主觀沒變順，且圖片恢復清晰變慢 |
| 移動期間暫停原圖 queue | 沒有明顯改善，清晰速度變慢 |
| 每張卡片加 `will-change` | 沒有改善，增加 layer 與 GPU memory 壓力 |
| `contain: paint` | 沒有明顯改善 |
| 移動時隱藏 grid 與 labels | 主觀略順，但解決不了主要卡頓（此項保留至今） |
| 平移期間只更新 world transform | 減少非必要工作，但 raster／composite 仍是瓶頸 |
| 已載入原圖時移動中改顯示快取縮圖 | 不解決卡頓，且移動後約 0.5 秒才清晰 |
| 平移不再提升整個 `.world` layer | 使用者回報仍然會卡 |

共同點：**調整的都是 composite 與載入時機，而成本在 raster worker 的解碼。**
...
後續實機 trace 顯示 250ms 的 coverage 間隔會讓新進畫面的縮圖明顯晚出現，因此目前縮短為 120ms；pass 仍只做媒體覆蓋，且移動期間不啟動新原圖載入。
...
### 第十一輪：圖片管線到此為止（`Profile-20260809T193545.json`）

`0c56d31`（lead 只服務縮圖）**沒有可量測的效果**：每 250ms 解碼峰值 13 → 12，突發中的解碼 59 → 58。該改動在原則上仍成立，但不應被記為改善。

這一輪改用 frame pipeline 而非執行緒總量來判讀，結論是圖片管線已經不是原因：

```text
present 間隔中位數   16.67 ms  →  60.0 fps
97% 的畫格間隔       = 16.67 ms
掉幀                 165 / 約 3061 (5.4%)，集中在 34 次 hitch
```

**hitch 與母檔解碼不相關。** 用相同長度的隨機視窗做對照：

```text
hitch 與 >100ms 解碼重疊    35%
隨機視窗基準率              28%
「任何解碼」重疊 38% vs 基準 37%
```

而 CPU profile 顯示 hitch 期間主執行緒 **80% 閒置**（正常時段 68.9%），我們的 JS 合計約 4%（最大是 `step @camera-navigation.js` 3.7%）。
...
- **單一視窗的巧合會騙人；用隨機視窗建立基準率。** 我從一個 hitch 觀察到「139ms 阻塞 + 293ms 解碼同時發生」而歸因於解碼，對照後發現重疊率與隨機無異。這個 session 裡同類錯誤發生過多次。
- **要判斷掉幀原因，先讀 `PipelineReporter` 與 present 間隔**，那是權威來源；執行緒總量只能提出假設。

### 下一步應該是 A/B，不是再改一次

與最初「隱藏 `.media-card`」那次同樣的做法：錄一段「原圖全部載入完成後、只做平移」的 trace，期間不應有任何新的原圖載入。若 hitch 照樣出現，即可確定與圖片無關，該轉向 Eagle／Electron 主機、GPU 驅動或素材所在磁碟。
